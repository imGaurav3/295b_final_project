...
router.post('/submit', async (req, res) => {
  ...
  if ( !user_id || !mood || !recommOption || !chipGroup || !chipTime) {
    return res.status(HttpCodes.BadRequest).json({ errmessage: 'Please fill out all the fields.' });
  }
  ...
  try {
    ...
    pool.then((p) => {
      ...
      return p.getConnection().then((connection) => {
        ...
        return connection.query(insertSql, values).then((results) => {
          if (results.affectedRows === 0) {
            throw new Error('New questionnaire response has not been added.');
          }
          ...
          res.status(HttpCodes.OK).json({ message: 'Questionnaire responses stored successfully.' });
        }).catch((err) => {
    ...
    console.error(err);
    res.status(HttpCodes.InternalServerError).json({ errmessage: 'Error storing questionnaire responses.' });
  }
});
